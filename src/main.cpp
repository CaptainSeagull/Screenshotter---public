...
handle_input_and_render(API *api) {
    ...
    if(api->init) {
        ...
        push_solid_rectangle(renderer, &renderer->root,
                             100, 100, 100, 100,
                             255, 0, 0, 0);
        Render_Entity *sudo_window = push_solid_rectangle(renderer, &renderer->root,
                                                          200, 100, 350, 300,
                                                          255, 0, 255, 0);
        Render_Entity *sudo_window2 = push_solid_rectangle(renderer, &sudo_window,
                                                           50, 50, 256, 128,
                                                           0, 0, 255, 0);
        ...
        push_image_rect(renderer, &sudo_window2,
                        0, 0, 64, 64,
                        0, 0, 64, 64,
                        arrow_id);
        push_image_rect(renderer, &sudo_window2,
