...
static uint64_t type_to_size(Type type) {
    switch(type) {
        ...
        case Type_stbtt__buf: { return(sizeof(stbtt__buf)); } break;
...
static Type string_to_type(char const *s, uint64_t l) {
    ...
    else if(generated_string_compare("stbtt__buf", 10, s, l)) { return(Type_stbtt__buf); }
    else if(generated_string_compare("stbtt_pack_context", 18, s, l)) { return(Type_stbtt_pack_context); }
    else if(generated_string_compare("stbtt_fontinfo", 14, s, l)) { return(Type_stbtt_fontinfo); }
    else if(generated_string_compare("stbrp_rect", 10, s, l)) { return(Type_stbrp_rect); }
    else if(generated_string_compare("stbtt_pack_context", 18, s, l)) { return(Type_stbtt_pack_context); }
    else if(generated_string_compare("stbtt_fontinfo", 14, s, l)) { return(Type_stbtt_fontinfo); }
    else if(generated_string_compare("stbtt_kerningentry", 18, s, l)) { return(Type_stbtt_kerningentry); }
    ...
    else if(generated_string_compare("DLL_Data", 8, s, l)) { return(Type_DLL_Data); }
    return(Type_unknown);
}
...
#if !defined(override_print_type_stbtt__buf)
static void print_type_stbtt__buf(char *buf, int *written, int max_size, stbtt__buf *param, char *name) {
    int r = SNPRINTF(buf + *written, max_size - *written, "stbtt__buf %s:\n", name);
    if(r != -1) { *written += r; }

    if(param->data) { print_type_uint8_t(buf, written, max_size, (uint8_t *)param->data, "data"); }
    else { int r2 = SNPRINTF(buf + *written, max_size - *written, "uint8_t data: (NULL)\n"); if(r2 != -1) { *written += r2; } }
    print_type_int(buf, written, max_size, (int *)&param->cursor, "cursor");
    print_type_int(buf, written, max_size, (int *)&param->size, "size");
}
#endif
static int print_type(char *buf, int max_size, stbtt__buf *param) {
    int written = 0;
    print_type_stbtt__buf(buf, &written, max_size, param, "");
    return(written);
}

#if !defined(override_print_type_stbtt_pack_context)
static void print_type_stbtt_pack_context(char *buf, int *written, int max_size, stbtt_pack_context *param, char *name) {
    int r = SNPRINTF(buf + *written, max_size - *written, "stbtt_pack_context %s:\n", name);
    if(r != -1) { *written += r; }

    print_type_int(buf, written, max_size, (int *)&param->width, "width");
    print_type_int(buf, written, max_size, (int *)&param->height, "height");
    print_type_int(buf, written, max_size, (int *)&param->stride_in_bytes, "stride_in_bytes");
    print_type_int(buf, written, max_size, (int *)&param->padding, "padding");
    print_type_int(buf, written, max_size, (int *)&param->skip_missing, "skip_missing");
    print_type_uint32_t(buf, written, max_size, (uint32_t *)&param->h_oversample, "h_oversample");
    if(param->pixels) { print_type_uint8_t(buf, written, max_size, (uint8_t *)param->pixels, "pixels"); }
    else { int r2 = SNPRINTF(buf + *written, max_size - *written, "uint8_t pixels: (NULL)\n"); if(r2 != -1) { *written += r2; } }
}
#endif
static int print_type(char *buf, int max_size, stbtt_pack_context *param) {
    int written = 0;
    print_type_stbtt_pack_context(buf, &written, max_size, param, "");
    return(written);
}

#if !defined(override_print_type_stbtt_fontinfo)
static void print_type_stbtt_fontinfo(char *buf, int *written, int max_size, stbtt_fontinfo *param, char *name) {
    int r = SNPRINTF(buf + *written, max_size - *written, "stbtt_fontinfo %s:\n", name);
    if(r != -1) { *written += r; }

    if(param->data) { print_type_uint8_t(buf, written, max_size, (uint8_t *)param->data, "data"); }
    else { int r2 = SNPRINTF(buf + *written, max_size - *written, "uint8_t data: (NULL)\n"); if(r2 != -1) { *written += r2; } }
    print_type_int(buf, written, max_size, (int *)&param->fontstart, "fontstart");
    print_type_int(buf, written, max_size, (int *)&param->numGlyphs, "numGlyphs");
    print_type_int(buf, written, max_size, (int *)&param->loca, "loca");
    print_type_int(buf, written, max_size, (int *)&param->index_map, "index_map");
    print_type_int(buf, written, max_size, (int *)&param->indexToLocFormat, "indexToLocFormat");
    print_type_stbtt__buf(buf, written, max_size, (stbtt__buf *)&param->cff, "cff");
    print_type_stbtt__buf(buf, written, max_size, (stbtt__buf *)&param->charstrings, "charstrings");
    print_type_stbtt__buf(buf, written, max_size, (stbtt__buf *)&param->gsubrs, "gsubrs");
    print_type_stbtt__buf(buf, written, max_size, (stbtt__buf *)&param->subrs, "subrs");
    print_type_stbtt__buf(buf, written, max_size, (stbtt__buf *)&param->fontdicts, "fontdicts");
    print_type_stbtt__buf(buf, written, max_size, (stbtt__buf *)&param->fdselect, "fdselect");
}
#endif
static int print_type(char *buf, int max_size, stbtt_fontinfo *param) {
    int written = 0;
    print_type_stbtt_fontinfo(buf, &written, max_size, param, "");
    return(written);
}

#if !defined(override_print_type_stbtt_kerningentry)
static void print_type_stbtt_kerningentry(char *buf, int *written, int max_size, stbtt_kerningentry *param, char *name) {
    int r = SNPRINTF(buf + *written, max_size - *written, "stbtt_kerningentry %s:\n", name);
    if(r != -1) { *written += r; }

    print_type_int(buf, written, max_size, (int *)&param->glyph1, "glyph1");
    print_type_int(buf, written, max_size, (int *)&param->glyph2, "glyph2");
    print_type_int(buf, written, max_size, (int *)&param->advance, "advance");
}
#endif
static int print_type(char *buf, int max_size, stbtt_kerningentry *param) {
    int written = 0;
    print_type_stbtt_kerningentry(buf, &written, max_size, param, "");
    return(written);
}
...
#if !defined(override_print_type_Image_Letter_Result)
static void print_type_Image_Letter_Result(char *buf, int *written, int max_size, Image_Letter_Result *param, char *name) {
    int r = SNPRINTF(buf + *written, max_size - *written, "Image_Letter_Result %s:\n", name);
    if(r != -1) { *written += r; }

    print_type_Image_Letter(buf, written, max_size, (Image_Letter *)&param->image_letter, "image_letter");
    print_type_Int(buf, written, max_size, (Int *)&param->success, "success");
}
#endif
static int print_type(char *buf, int max_size, Image_Letter_Result *param) {
    int written = 0;
    print_type_Image_Letter_Result(buf, &written, max_size, param, "");
    return(written);
}

#if !defined(override_print_type_Entry)
static void print_type_Entry(char *buf, int *written, int max_size, Entry *param, char *name) {
    ...
    print_type_Window_Info(buf, written, max_size, (Window_Info *)&param->info, "info");
}
#endif
static int print_type(char *buf, int max_size, Entry *param) {
    int written = 0;
    print_type_Entry(buf, &written, max_size, param, "");
    return(written);
}

#if !defined(override_print_type_DLL_Data)
static void print_type_DLL_Data(char *buf, int *written, int max_size, DLL_Data *param, char *name) {
    int r = SNPRINTF(buf + *written, max_size - *written, "DLL_Data %s:\n", name);
    if(r != -1) { *written += r; }
...
static int print_type(char *buf, int max_size, DLL_Data *param) {
    int written = 0;
    print_type_DLL_Data(buf, &written, max_size, param, "");
    return(written);
}
